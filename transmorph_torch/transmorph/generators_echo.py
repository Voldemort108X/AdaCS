...
def volgen_echo(
    ...
    while True:
        ...
        imgs_ED, imgs_ES = [], []
        for i in indices:
            im_ED, im_ES = py.utils_echo.load_volfile_mat(vol_names[i], **load_params)
...
def scan_to_scan_echo(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    ...
    """
    zeros = None
    gen = volgen_echo(vol_names, batch_size=batch_size, **kwargs)
    while True:
        scan1 = next(gen)[0]
        scan2 = next(gen)[0]
        # print(scan1.shape)
...
def volgen_echo_3frame(
    ...
    while True:
        ...
        imgs_ED, imgs_ES, imgs_mid = [], [], []
        for i in indices:
            im_ED, im_ES, im_mid = py.utils_echo.load_volfile_mat_3frame(vol_names[i], **load_params)
            imgs_ED.append(im_ED), imgs_ES.append(im_ES), imgs_mid.append(im_mid)
        
        vols_ED, vols_ES, vols_mid = [np.concatenate(imgs_ED, axis=0)], [np.concatenate(imgs_ES, axis=0)], [np.concatenate(imgs_mid, axis=0)]
        ...
        yield tuple(vols_ED)
        yield tuple(vols_ES)
        yield tuple(vols_mid)


def scan_to_scan_echo_3frame(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan registration.
    ...
    """
    zeros = None
    gen = volgen_echo_3frame(vol_names, batch_size=batch_size, **kwargs)
    while True:
        scan1 = next(gen)[0]
        scan2 = next(gen)[0]
        scan3 = next(gen)[0]
        ...
        invols_EDES = [scan1, scan2]
        outvols_EDES = [scan2, scan1] if bidir else [scan2]
        if not no_warp:
            outvols_EDES.append(zeros)

        invols_EDmid = [scan1, scan3]
        outvols_EDmid = [scan3, scan1] if bidir else [scan3]
        if not no_warp:
            outvols_EDmid.append(zeros)

        invols_midES = [scan3, scan2]
        outvols_midES = [scan2, scan3] if bidir else [scan2]
        if not no_warp:
            outvols_midES.append(zeros)

            
        yield (invols_EDES, outvols_EDES, invols_EDmid, outvols_EDmid, invols_midES, outvols_midES)
        #yield (invols, outvols)