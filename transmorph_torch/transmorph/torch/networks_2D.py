...
class TransMorph(LoadableModel):
    ...
    @store_config_args
    def __init__(self, config, bidir):
        '''
        TransMorph Model
        '''
    ...
    def forward(self, source, target, registration=False):
        # source = x[:, 0:1, :, :] # only the first image

        # concatenate inputs and propagate unet
        x = torch.cat([source, target], dim=1)

        # get the background mask for zero-valued entries for ultrasound images
        # mask_bgd = (source - target)!=0
        mask_bgd = (source!=0) + (target!=0)

        # propagate network to get 2D displacement
        if self.if_convskip:
            x_s0 = x.clone()
        ...
        pos_flow = self.reg_head(x)

        # negate flow to bidirectional model
        neg_flow = -pos_flow if self.bidir else None

        y_source = self.spatial_trans(source, pos_flow)
        y_target = self.spatial_trans(target, neg_flow) if self.bidir else None

        if not registration:
            return (y_source, y_target, pos_flow, mask_bgd) if self.bidir else (y_source, pos_flow, mask_bgd)
        else:
            return y_source, pos_flow, mask_bgd
