...
model = vxm.networks.VxmDense(inshape=inshape).load(args.model, device) # size = (64,64,64) inshape inside Vxm: (128, 128, 128)
model.to(device)
model.eval()
...
for file_path in test_files:
    ...
    im_ES_pred, dvf, mask_bgd = model(input_ED, input_ES, registration=True)
