...
parser.add_argument('--momentum_scaling', type=float, default=1000, help='decay rate of momentum')
...
# parser.add_argument('--mom_guide', action='store_true', help='use momentum to guide the scale of the weighting')
...
assert args.motion_loss_type in ['mse', 'wmse']
...
if args.load_model_covis:
    # load initial model (if specified)
    model_covis = vxm.networks.CoVisNet.load(args.load_model_covis, device)
else:
    # otherwise configure new model
    model_covis = vxm.networks.CoVisNet(
        inshape=inshape,
        nb_unet_features=[enc_nf, dec_nf],
        bidir=bidir,
        int_steps=args.int_steps,
...
if args.covis_loss_type == 'coviswmse':
    ...
    loss_func_covis = vxm.losses.CoVisWeightedMSE().loss
...
losses_covis += [vxm.losses.ScoringLoss().loss]
...
artifact_covis = wandb.Artifact('voxelmorph_covis', type='model')
