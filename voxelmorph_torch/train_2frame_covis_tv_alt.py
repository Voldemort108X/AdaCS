#!/usr/bin/env python
################################################
# alternative formulation of accepting two input
##################################################
...
losses_covis += [vxm.losses.CoVisTVLoss(ndims = ndims).loss]
    ...
    for step in range(args.steps_per_epoch):
            ...
            for n, loss_function in enumerate(losses_motion):
                ...
                if n < len(losses_motion) - 1:
                    ...
                    if flag_covis == False:
                        ...
                        scoring_mask = model_covis(input_bank) # (input_ES, input_ED)
        ...
        if flag_covis == True:
            ...
            for idx_bidir in range(num_directions):
                input_bank = torch.cat((y_pred[idx_bidir], y_true[idx_bidir]), dim=1)
