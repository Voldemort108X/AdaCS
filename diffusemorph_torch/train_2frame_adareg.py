    ...
    if opt['phase'] == 'train':
        ...
        while current_epoch < n_epoch:
            ...
            for istep in range(args.steps_per_epoch):
                ...
                train_data = {'M': inputs[0], 'F': y_true[0], 'reg_mask': torch.ones_like(y_true[0])}

                diffusion.feed_data_2frame(train_data)
                y_pred, mask_bgd = diffusion.forward_2frame_adaframe()

                reg_mask = compute_reg_mask(y_pred, y_true[0], mask_bgd, args.c)

                train_data = {'M': inputs[0], 'F': y_true[0], 'reg_mask': reg_mask}
            ...
            logs["pred_reg_mask"] = pred_reg_mask
            logs["pred_reg_mask_mean"] = np.mean(reg_mask.cpu().detach().numpy()[0, 0])
