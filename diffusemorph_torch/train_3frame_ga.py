...
if __name__ == "__main__":
    ...
    if args.dataset == 'Echo':
        generator = data_gen.generators_echo.scan_to_scan_echo_3frame(
    ...
    if opt['phase'] == 'train':
        ...
        while current_epoch < n_epoch:
            ...
            for istep in range(args.steps_per_epoch):
                ...
                train_data_EDES = {'M': inputs_EDES[0], 'F': y_true_EDES[0]}
                train_data_EDmid = {'M': inputs_EDmid[0], 'F': y_true_EDmid[0]}
                train_data_midES = {'M': inputs_midES[0], 'F': y_true_midES[0]}

                current_step += 1

                diffusion.feed_data_3frame(train_data_EDES, train_data_EDmid, train_data_midES)

                # print(current_step)

                
                # diffusion.optimize_parameters_3frame()
                diffusion.optimize_parameters_3frame_grad_accum(idx_step=istep, grad_accum_step=args.accumulation_steps)
