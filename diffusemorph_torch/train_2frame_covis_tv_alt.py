            ...
            for istep in range(args.steps_per_epoch):
                ...
                if flag_motion == True:
                    ...
                    if flag_covis == False:
                        ...
                        train_data_covis = {'M': inputs[0], 'F': y_true[0], 'scoring_mask':  torch.ones_like(y_true[0])}
                        diffusion.feed_data_2frame(train_data_covis)
                        y_pred_covis = diffusion.forward_2frame()

                        # print(y_pred_covis.shape)

                        input_bank = torch.cat((y_pred_covis, y_true[0]), dim=1)
                ...
                if flag_covis == True:
                    ...
                    # print(y_pred.shape)
                    input_bank = torch.cat((y_pred, y_true[0]), dim=1)
                    ...
                    for n, loss_function in enumerate(losses_covis):
                        ...
                        # for idx_bidir in range(num_directions):
                        curr_loss = loss_function(y_true[0], y_pred, scoring_mask, y_pred[-1]) * weights_covis[n]
