...
def volgen_clinical_echo(
    ...
    while True:
        ...
        imgs_ED, imgs_ES = [], []
        for i in indices:
            im_ED, im_ES = py.utils_clinical_echo.load_volfile_mat(vol_names[i], **load_params)
...
def scan_to_scan_clinical_echo(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    ...
    """
    zeros = None
    gen = volgen_clinical_echo(vol_names, batch_size=batch_size, **kwargs)
    while True:
        scan1 = next(gen)[0]
        scan2 = next(gen)[0]
        # print(scan1.shape)
