...
def load_volfile_mat(
    ...
    if filename.endswith('.mat'):
        ...
        if 'ED_idx' in file.keys():
            ...
            # for synthetic data and canine
            numOfFrames = file['im4D_resize'].shape[-1]
            # we can modify this later but let us just assume it now
            ED_idx = 0
...
# 3-frame version of load_volfile_mat
def load_volfile_mat_3frame(
    filename,
    np_var='vol',
    add_batch_axis=False,
    add_feat_axis=False,
    ...
    if filename.endswith('.mat'):
        ...
        if 'ED_idx' in file.keys():
            im_ED = file['im4D_resize'][:, :, :, int(file['ED_idx'].squeeze())]
            im_ES = file['im4D_resize'][:, :, :, int(file['ES_idx'].squeeze())]
            im_mid = file['im4D_resize'][:, :, :, int((int(file['ED_idx'].squeeze())+int(file['ES_idx'].squeeze()))/2)]
            ...
            im_ED = file['im4D_resize'][:, :, :, ED_idx]
            im_ES = file['im4D_resize'][:, :, :, ES_idx]
            im_mid = file['im4D_resize'][:, :, :, int(ED_idx+ES_idx)//2]
    ...
    if pad_shape:
        vol_ED, _ = pad(vol_ED, pad_shape)
        vol_ES, _ = pad(vol_ES, pad_shape)
        vol_mid, _ = pad(vol_mid, pad_shape)
    ...
    if resize_factor != 1:
        vol_ED = resize(vol_ED, resize_factor)
        vol_ES = resize(vol_ES, resize_factor)
        vol_mid = resize(vol_mid, resize_factor)
    ...
    if registration:
        ...
        return (vol_ED, vol_ES, vol_mid, affine) if ret_affine else (vol_ED, vol_ES, vol_mid)